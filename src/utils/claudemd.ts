 ...
 * 2. User memory (~/.doge/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (CLAUDE.md, .doge/CLAUDE.md, and .doge/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - CLAUDE.md, .doge/CLAUDE.md, and all .md files in .doge/rules/ are checked in each directory for Project memory
 *
...
/**
 * Processes all .md files in the .doge/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .doge/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.doge/rules/*.md files
    ...
    // .doge/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like CLAUDE.md and .doge/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .doge/CLAUDE.md (Project)
        ...
        // Try reading .doge/rules/*.md files (Project)
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .doge/CLAUDE.md from the additional directory
        const dotClaudePath = join(dir, '.claude', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .doge/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.claude', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .doge/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .doge/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (CLAUDE.md and .doge/CLAUDE.md)
  ...
  // Process project unconditional .doge/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .doge/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
/**
 * Check if a file path is a memory file (CLAUDE.md, CLAUDE.local.md, or .doge/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .doge/rules/ directories
  if (
    name.endsWith('.md') &&
